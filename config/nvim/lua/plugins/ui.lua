...
return {
  ...
  -- Description: A blazing fast and easy to configure Neovim statusline plugin.
  {
    ...
    requires = { "nvim-tree/nvim-web-devicons", opt = true }, -- Optional dependency for icons
      opts = {
        options = {
          theme = "auto", -- adapt to colorscheme (latte/nude style)
          icons_enabled = false, -- no Nerd Font dependency
        },
        sections = {
          lualine_a = {
            {
              "mode", -- Display the current mode
              icon = "MODE", -- clean text icon
            },
          },
        },
      extensions = {
  ...
  -- Description: A Neovim plugin for creating a customizable dashboard.
  {
    ...
    opts = {
      ...
      dashboard = {
        sections = {
          { section = "header" },
          { icon = "[K] ", title = "Keymaps", section = "keys", indent = 2, padding = 1 },
          { icon = "[F] ", title = "Recent Files", section = "recent_files", indent = 2, padding = 1 },
          { icon = "[P] ", title = "Projects", section = "projects", indent = 2, padding = 1 },
          { section = "startup" },
        },
        preset = {
          header = [[
      Gentleman's Nude Workspace
    Clean • Fast • Focused • Stable
]],
          -- stylua: ignore
          ---@type snacks.dashboard.Item[]
          keys = {
            { icon = "[F] ", key = "ff", desc = "Find File", action = ":lua Snacks.dashboard.pick('files')" },
            ...
            { icon = "[X] ", key = "x", desc = "Lazy Extras", action = ":LazyExtras" },
