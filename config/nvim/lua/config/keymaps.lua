...
-- OpenCode (Inteligente para buffers normales, Oil y mini.files)
vim.keymap.set("n", "<leader>oo", function()
  local path = ""
  
  -- Verificar si estamos en mini.files
  local is_mini_files = vim.bo.filetype == "minifiles"
  if is_mini_files then
    local entry = require("mini.files").get_fs_entry()
    if entry then
      path = entry.fs_type == "directory" and entry.path or vim.fn.fnamemodify(entry.path, ":h")
    end
  else
    -- Para Oil o archivos normales
    path = vim.fn.expand("%:p")
    if path == "" or string.match(path, "^oil://") then
  ...
  vim.cmd("resize 15")
  vim.cmd("terminal cd " .. vim.fn.shellescape(path) .. " && opencode .")
  vim.cmd("startinsert")
end, { desc = "OpenCode en el directorio actual" })
